...
class CassieData {
  // sjy edit: add more parameters in the ctor to add yaml support
  public:
    CassieData(ros::NodeHandle& nh, std::string static_frame, std::string smap_topic, std::string tmap_topic,
        ...
        // sjy edit
        double search_distance = 2.0, double cos_view = cos(Pi / 3), 
        double pitch = 0 * Pi / 3, double sm = 2.0,
        double prob_stat = 0.75, double delta_ric = 0.01,
        double budget = 5, double bb_blocks_size = 5, double view_angle = PI/3
        )
    : nh_(nh)
    ...
    , max_range_(max_range) {
      ...
      iig_map_param.search_distance = search_distance;
      // iig_map_param.res = 0.1; // voxel
      iig_map_param.cos_view = cos_view; // 90 deg
      iig_map_param.pitch = pitch;// - Pi / 6; // rad
      iig_map_param.sm = sm;
      iig_map_param.line_search_step = iig_map_param.res * 0.4;// * 2;
      iig_map_param.line_search_thres = iig_map_param.res * 0.65;
      iig_map_param.prob_sat = prob_stat;

      iig_planner_param.delta_ric = delta_ric; //0.008;
      iig_planner_param.budget = budget;

      bb_blocks_size_ = bb_blocks_size;
    ...
    void publish_path() {
        ...
        la3dm::point3f bb_min(RobotPose_now.x-bb_blocks_size_, RobotPose_now.y-bb_blocks_size_, RobotPose_now.z-2);
        la3dm::point3f bb_max(RobotPose_now.x+bb_blocks_size_, RobotPose_now.y+bb_blocks_size_, RobotPose_now.z+2);
        ...
        for (auto it = bb_blocks.begin(); it != bb_blocks.end(); ++it)
        {
          ...
          //
          if (abs(atan2(y_block_converted, x_block_converted)) > view_angle_ || sqrt(x_block_converted*x_block_converted + y_block_converted*y_block_converted) > bb_blocks_size_/sqrt(2))
          {
            continue_block++;
            continue;
          }
  ...
  private:
    ...
    // sjy edit add more params
    double bb_blocks_size_;
    double view_angle_;
