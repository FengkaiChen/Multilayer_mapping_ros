...
int main(int argc, char **argv) {
    ...
    // convert the data to iig format. 
        // la3dm::point3f lim_min(0,0,-2), lim_max(20, 10, 4);

        // vector<la3dm::BlockHashKey> blocks;
        // map.get_blocks_in_bbox(lim_min, lim_max, blocks);

    ofstream obs3("/home/sangli/Research/BipedIIG/BipedIIG/testdata/map/sdata.txt");
    int num_class = 4;

    MapParam mparam;
    SemanticMap pMap(mparam, num_class);
    int num_voxels = 0;
    for (auto it = map.begin_leaf(); it != map.end_leaf(); ++it) {
      if (it.get_node().get_state() == la3dm::State::OCCUPIED) {
            if (original_size) {
                vector<float> prob_semantic(num_class);// not sure how many clases are there. 
                vector<float> vars_semantic(num_class);// not sure how many clases are there. 
                ...
                for(int jj = 0; jj < num_class; jj++) {
                    prob_semantic[jj] = (double)prob_semantic[jj];
                    vars_semantic[jj] = (double)vars_semantic[jj];
                }
                pMap.insertGrid_2(p.x(), p.y(), p.z(), prob_semantic, vars_semantic, traversability<0.70);
                obs3<<p.x()<<" "<<p.y()<<" "<<p.z()<<" ";
    ...
    pMap.buildTree();
    cout<<"Number of Voxels = "<<num_voxels<<endl;
    ...
    // pa.budget = 40;
    // void *map;
    SemanticIIG planner(init, &pMap, pa);
    planner.Plan();
